    ...
    <div class="col-xl">
        <div class="card mb-4">
            <div class="card-header d-flex justify-content-between align-items-center">
                <h5 class="mb-0">{{ __('label.edit_bike') }}</h5>
            </div>
            <div class="card-body">
                <form method="post" action="{{ route('bike.update', $motor->id) }}" class="form-block"
                    ...
                    <div class="row mb-3">
                        <div class="col-md-4">
                            <x-form.input-text name="name" :label="__('label.name')" :old="old('name', $motor->name)" />
                        </div>
                        <div class="col-sm-6 col-md-3">
                            <x-form.input-group-mask name="plate" :label="__('label.plate')" :old="old('plate', $motor->plate)" mask="plate"
                            ...
                            <select id="brand_id" name="brand_id" class="form-select">
                                ...
                                @foreach ($brand as $item)
                                    <option value="{{ $item->id }}"
                                        {{ old('brand_id', $motor->brand_id) == $item->id ? 'selected' : '' }}>
                                        {{ $item->name }}
                                    </option>
                                @endforeach
                            </select>
                    ...
                    <div class="mb-3">
                        <div class="col-md-4">
                            <x-form.input-text name="color" :label="__('label.color')" :old="old('color', $motor->color)" />
                        </div>
                    </div>
                    <div class="mb-3">
                        <div class="col-md-4">
                            <x-form.input-group-mask name="price" :label="__('label.price')" :old="old('price', $motor->price)"
                    ...
                    <div class="row mb-3">
                        <div class="col-sm-6 col-md-6">
                            ...
                            @if ($motor->image)
                                <div class="mb-3">
                                    <img src="{{ asset('storage/' . $motor->image) }}" alt="Current Logo" class="img-fluid"
                    ...
                    <div class="row mb-3">
                        <div class="col-sm-6 col-md-6">
                            <x-input-text name="description" label="{{ __('label.description') }}" icon="bx bx-text"
                                placeholder="Enter your description" value="{{ old('description') }}" :value="$motor->description" />
                        </div>
                    </div>
...
@push('scripts')
    <script>
        ...
        $(document).ready(function() {
            ...
            const plateInput = $(".indonesianPlate-mask");

            const initialValue = plateInput.val();

            plateInput.inputmask({
